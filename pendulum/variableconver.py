...
def send_update(label, c):
    time.sleep(0.5)
    c.set_multiplier(label, 0.5)
    print("Setting multiplier")
...
control = sim.external_devices.SpynnakerConvolutionControlConnection(
    pop_labels=[POP_LABEL], local_port=None)
control.add_start_callback(POP_LABEL, send_update)
...
sim.external_devices.add_convolution_multiplier_control(
    target_pop, database_notify_port_num=control.local_port)
