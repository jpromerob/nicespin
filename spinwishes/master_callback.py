...
def start_receiver(args, q_sender, q_receiver, q_data):
    ...
    while(True):
        ...
        if value == "start_receiver":
            ...
            ev_count = struct.unpack('i', data)[0]
            print(f"received ev_count = {ev_count}")
            # try:
...
def start_sender(args, q_sender, q_receiver, q_data):
    ...
    sleeper_base_list = np.concatenate(([2000000, 2000000],np.logspace(6.31,4,20), np.logspace(3.9,1.6,300), np.logspace(1.6,0,20)))
    # sleeper_base_list = [444, 192]
    # s_64_list = np.concatenate(([2000000, 2000000], np.logspace(6.31,4,28), np.logspace(4,0, 20)))
    # s_256_list = np.logspace(2.25,0, len(s_64_list))
    # sleeper_base_list = np.concatenate((s_64_list, s_256_list))
    # pack_sz_list = np.concatenate((np.ones(len(s_64_list))*64, np.ones(len(s_256_list))*256))

    # sleeper_base_list = np.concatenate(([2000000, 2000000], np.logspace(6.31,4,28), np.logspace(4,0, 20)))
    # sleeper_base_list = [444, 192, 107, 65, 37, 23, 11, 2, 1]
    pack_sz_list = np.ones(len(sleeper_base_list))*256
    ...
    while(True):
        ...
        if value == "start_sender":
            time.sleep(1)
            cmd = f"./c_code/send_and_request.exe {args.width} {args.height} {int(sleeper_base_list[idx])} {args.spif_ip}:3333 4000 1 {int(pack_sz_list[idx])}"
            print(f"Start Sender: {cmd}")
            output = subprocess.check_output(cmd, shell=True)
            ev_data = output.decode('utf-8')
            q_data.put((0,ev_data[0:-1]))
            time.sleep(args.exduration)
            # cmd = f"./c_code/send_and_request.exe {args.width} {args.height} 1000000 {args.spif_ip}:3333 4000 1 1 "
            # os.system(cmd) 
            time.sleep(1)
...
def print_data(args, q_data):
    ...
    while True:
        ...
        if tcp_dump_val != "" and sender_val != "":
            ...
            print(f"\tRatio c++ vs SPIF(out): {round(100*int(val_array[7])/max(1,int(val_array[4]),2))}%")
            tcp_dump_val = ""
            sender_val = ""
...
def parse_args():
    ...
    parser.add_argument('-s', '--ev-per-pack', type=int, help="PC port", default=256)   
