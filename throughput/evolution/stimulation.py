...
class Stimulator:
    def __init__(self, args, input_q, end_of_sim):
        ...
        # Other Stuff
        self.bin = args.bin    
        # self.t_sleeper_array = np.logspace(2,-1, num=200, base=10) # [0.1, 0.01, 0.001] #
        # self.t_sleeper_array = np.concatenate(([2000000, 2000000],np.logspace(6.31,4,20), np.logspace(3.9,1.6,500), np.logspace(1.6,0,20)))
        # self.t_sleeper_array = np.concatenate((np.logspace(3.9,1.6,500), np.logspace(1.6,0,20)))
        self.t_sleeper_array = np.logspace(1.6,0,20)
        self.spin_waiter = 30
        self.time_per_point = 2*self.bin
        self.duration = 300+self.time_per_point*len(self.t_sleeper_array) + self.spin_waiter
        ...
        while self.end_of_sim.value == 0:
            ...
            for i in range(pack_sz):   
                ...
                if t_current >= t_start + self.bin:
                    ...
                    expected_count = int((1000000/t_sleeper) * pack_sz)
