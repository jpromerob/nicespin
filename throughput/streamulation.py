...
def get_ev_count_list(t_wait_list):
    
    ev_count_array = []
    with open("ev_count.txt") as fp:
        for line in fp:
            ev_count_array.append(int(line.strip().replace("Sent a total of ", "").replace(" events", "")))
    ev_count_array = np.asarray(ev_count_array, dtype="int")

    t_wait_array = []
    with open("t_wait.txt") as fp:
        for line in fp:
            t_wait_array.append(int(line.strip()))
    t_wait_array = np.asarray(t_wait_array, dtype="int")


    ev_count_list = []
    for t_wait in t_wait_list:
        l_idx = np.where(t_wait_array >= t_wait)[0][-1]
        if l_idx < len(t_wait_array)-1:
            h_idx = l_idx + 1
            x1 = t_wait_array[l_idx] 
            y1 = ev_count_array[l_idx] 
            x2 = t_wait_array[h_idx] 
            y2 = ev_count_array[h_idx]
            m = (y2-y1)/(x2-x1)
            b = y1-m*x1
            val = round(m*t_wait+b,0)
        else:
            val = ev_count_array[l_idx]
        ev_count_list.append(val)
        
    
    return ev_count_list
...
class Streamulator:
    def __init__(self, args, input_q, end_of_sim):
        ...
        if self.mode[0] == 's':
            ...
            self.duration = 24*60 # minutes
            self.t_wait_list = np.asarray(np.round(np.logspace(math.log(100,10),math.log(200000,10),int(self.duration/self.bin)),0),dtype='int')[::-1]
            self.ev_count_list = get_ev_count_list(self.t_wait_list) # @TODO !!!!
        if self.mode[0] == 'e':
            self.spikes = []
    ...
    def launch_input_handler(self):
        ...
        if self.stream:
            print("Using AEStream")
            count = 0
            while self.end_of_sim.value == 0:
                for idx in range(len(self.t_wait_list)):  
                    count += 1    
                    ev_per_s = int(self.ev_count_list[idx])
                    expected_count = ev_per_s
                    self.input_q.put((ev_per_s, expected_count), False)
                    print(f"Starting Streaming #{count}/{len(self.t_wait_list)}")
                    os.system(f"/opt/aestream/build/src/aestream input file w{self.width}h{self.height}l{self.length}t{self.t_wait_list[idx]}d{self.bin}.sim output udp {self.ip_addr} {self.spif_port}")
                    print("\n")
